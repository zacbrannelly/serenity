...
Vector<Gfx::Path> PaintableBox::compute_text_clip_paths(PaintContext& context) const
{
    ...
    for_each_in_inclusive_subtree([&](auto& paintable) {
        if (is<PaintableWithLines>(paintable)) {
            auto& paintable_lines = static_cast<PaintableWithLines const&>(paintable);
            for (auto& fragment : paintable_lines.fragments()) {
                if (is<Layout::TextNode>(fragment.layout_node())) {
                    auto const& text_node = static_cast<Layout::TextNode const&>(fragment.layout_node());
                    add_text_clip_path(text_node, fragment);
                }
            }
